...
<head>
    ...
    <title>All Property Rates | <?php echo $name; ?> </title>
    <!-- Favicon-->
                                ...
                                <table style="font-size:11px;" class="table small table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tbody>
                                      <?php
                                            ...
                                            $sql = "SELECT * FROM property WHERE activ = 'Y' ORDER BY id DESC";
                                            $result = $connect->query($sql);
                                          
                                            if($result->num_rows > 0) {
                                                while($row = $result->fetch_assoc()) {
                                                     $p_no = $row['pro_acct_no'];
                                                     $sql_ = "SELECT * FROM property_payment WHERE pro_acct_no='$p_no' AND activ = 'Y' order by id DESC";
                                                    $result_ = $connect->query($sql_);
                                                    $row_ = $result_->fetch_assoc();
                                                    $id_ = $row_['id'];

                                                     $sql_d = "SELECT * FROM property WHERE pro_acct_no='$p_no' AND activ = 'Y'";
                                                    ...
                                                    <td>" .$row['pro_owner_name']."</td>
                                                    <td>" .$row['pro_type']."</td>
                                                    <td>" .number_format($row['r_value'], 2, '.', ',')."</td>
                                                    <td>" .$row['r_impose']."</td>
                                                    <td>" .number_format($row['r_charge'], 2, '.', ',')."</td>
                                                    <td>" .$row['electoral_area']. "</td>
                                                    <td>" .$row['pro_address']. "</td>
                                                    <td>" .$row['pro_area_category']. "</td>
                                                    <td>" .$row['pro_hs_no']. "</td>
                                                     <td>
                                                    <a href='update-rate.php?id=".base64_encode($row['id'])."'>Edit</a>
                                                    <a href='print-bill-propertyr?id=".$id_."&p_no=".$row['pro_acct_no']."'>Print</a>
                                                     <a href='delete-pr?pr_id=".$id_d."&pro_acct_no=".$row_d['pro_acct_no']."'>Delete</a>
                                                                                                                </td>
                                                    </tr>";
                                                    $no++;
                                                }
