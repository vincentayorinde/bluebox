     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['bop_id']){
                  ...
                  $sql__ = "SELECT * FROM business WHERE b_no = '$bop_b_no' AND id= '$bop_id'";
                  $result__ = $connect->query($sql__);
                  $data__ = $result__->fetch_assoc();
    }
      if (isset($_POST['remove'])){
                    $sqlnew = "UPDATE business SET activ='N' WHERE id='$bop_id'";
                      $resultnew = $connect->query($sqlnew);
                      if($resultnew){
                        header("location:all-business");
                      }
                    }
    ?>
<!DOCTYPE html>
...
<body>
    	<div id="wrapper">
        <p> Are you sure you want to permanetly delete <strong><?php echo $data__['b_name'] ?></strong> with the ID <strong><?php echo $data__['b_no']; ?></strong> </p>
