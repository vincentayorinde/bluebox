     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['bp_id']){
                  ...
                  $sql__ = "SELECT * FROM building WHERE b_permit_acct_no = '$bp_no' AND id= '$bp_id'";
                  $result__ = $connect->query($sql__);
                  $data__ = $result__->fetch_assoc();
    }
      if (isset($_POST['remove'])){
                    $sqlnew = "UPDATE building SET activ='N' WHERE id='$bp_id'";
                      $resultnew = $connect->query($sqlnew);
                      if($resultnew){
                        header("location:all-building-permit");
                      }
                    }
    ?>
<!DOCTYPE html>
...
<body>
    	<div id="wrapper">
        <p> Are you sure you want to permanetly delete <strong><?php echo $data__['b_permit_owner_name'] ?></strong> with the ID <strong><?php echo $data__['b_permit_acct_no']; ?></strong> </p>
