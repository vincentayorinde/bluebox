    ...
    if(isset($_SESSION['is_logged_in']) != true){
     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['bop_id']){
                  $bop_id = $_GET['bop_id'];
                  $bop_b_no = $_GET['bop_b_no'];

                  $sql__ = "SELECT * FROM bop_payment WHERE b_no = '$bop_b_no' AND id= '$bop_id'";
                  $result__ = $connect->query($sql__);
                  $data__ = $result__->fetch_assoc();
    }
      if (isset($_POST['remove'])){
                    $sqlnew = "UPDATE bop_payment SET activ='N' WHERE id='$bop_id'";
                      $resultnew = $connect->query($sqlnew);
                      if($resultnew){
                        header("location:bop-payment-list");
                      }
                    }
    ?>
<!DOCTYPE html>
...
<body>
    	<div id="wrapper">
        <p> Are you sure you want to permanetly delete payment of <strong><?php echo $data__['paid'] ?></strong> for the ID <strong><?php echo $data__['b_name']; ?></strong> </p>
