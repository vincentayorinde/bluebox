     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['pr_id']){
                  ...
                  $sql__ = "SELECT * FROM property_payment WHERE pro_acct_no = '$pro_acct_no' AND id= '$pr_id'";
                  $result__ = $connect->query($sql__);
                  $data__ = $result__->fetch_assoc();
    }
      if (isset($_POST['remove'])){
                    $sqlnew = "UPDATE property_payment SET activ='N' WHERE id='$pr_id'";
                      $resultnew = $connect->query($sqlnew);
                      if($resultnew){
                        header("location:property-payment-list");
                      }
                    }
    ?>
<!DOCTYPE html>
...
<body>
    	<div id="wrapper">
        <p> Are you sure you want to permanetly delete payment of <strong><?php echo $data__['paid'] ?></strong> for the ID <strong><?php echo $data__['pro_acct_no']; ?></strong> </p>
