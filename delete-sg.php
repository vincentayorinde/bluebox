     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['sg_id']){
                  ...
                  $sql__ = "SELECT * FROM signage WHERE signage_acct_no = '$sg_no' AND id= '$sg_id'";
                  $result__ = $connect->query($sql__);
                  $data__ = $result__->fetch_assoc();
    }
      if (isset($_POST['remove'])){
                    $sqlnew = "UPDATE signage SET activ='N' WHERE id='$sg_id'";
                      $resultnew = $connect->query($sqlnew);
                      if($resultnew){
                        header("location:all-signage");
                      }
                    }
    ?>
<!DOCTYPE html>
...
<body>
    	<div id="wrapper">
        <p> Are you sure you want to permanetly delete <strong><?php echo $data__['signage_acct_name'] ?></strong> with the ID <strong><?php echo $data__['signage_acct_no']; ?></strong> </p>
