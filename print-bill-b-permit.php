    ...
    if(isset($_SESSION['is_logged_in']) != true){
     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['p_no']){
                  $b_permit_acct_no = $_GET['p_no'];
                  $id = $_GET['id'];

                   $sqlnewt = "SELECT sum(paid) as p_total,b_permit_acct_no FROM building_payment WHERE b_permit_acct_no = '$b_permit_acct_no'";
                  $resultnewt = $connect->query($sqlnewt);
                  $datat = $resultnewt->fetch_assoc();

           $sqlnew = "SELECT id,b_permit_acct_no,rate_impost,rate_charge,arrears,paid,bal,payment_mode,bank,cheque_no,gcr,user_id,user_email,date_added FROM building_payment WHERE b_permit_acct_no = '$b_permit_acct_no' AND id = '$id'";
                  ...
                  $sqlnewp = "SELECT * FROM building WHERE b_permit_acct_no = '$b_permit_acct_no'";
...
<H2 id="bop">BUILDING PERMIT BILL </H2> <strong> <span style="float: right;padding-right: 15px;">DATE: <?php echo date('Y-m-d'); ?></span></strong>
<table border="0" width="620" cellpadding="5">
	<tr>
		<td colspan="3"><strong>BILL TO:</strong> <?php echo $datap['b_permit_owner_name']; ?></td>
	</tr>
	<tr>
		<td colspan="3"><strong>BUILDING PERMIT ACCT. NO:</strong> <?php echo $data['b_permit_acct_no']; ?></td>
	</tr>
	<tr></tr>
	<tr>
		<td colspan="2"><strong>ELECTORAL LOCATION / AREA:</strong> <?php echo $datap['electoral_area']; ?></td>
		<td colspan="1"><strong>LOCATION:</strong><?php echo $datap['building_address']; ?></td>
	</tr>
	<tr>
		<td colspan="2"><strong>BUILDING TYPE: </strong><?php echo $datap['building_type']  ?></td>
...
<!-- 	<table border="0" width="620" cellpadding="5">
	...
	<tr>
		...
		<td><strong>Building Permit Rate</strong></td>
