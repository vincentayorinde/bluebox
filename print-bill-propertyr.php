    ...
    if(isset($_SESSION['is_logged_in']) != true){
     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['p_no']){
                  $pro_acct_no = $_GET['p_no'];
                  $id = $_GET['id'];

                   $sqlnewt = "SELECT sum(paid) as p_total,pro_acct_no FROM property_payment WHERE pro_acct_no = '$pro_acct_no'";
                  $resultnewt = $connect->query($sqlnewt);
                  $datat = $resultnewt->fetch_assoc();

           $sqlnew = "SELECT id,pro_acct_no,rateable_value,rate_impose,rate_charge,arrears,paid,bal,payment_mode,bank,cheque_no,gcr,discount,actual_discount,user_id,user_email,date_added FROM property_payment WHERE pro_acct_no = '$pro_acct_no' AND id = '$id'";
...
<html>
<head>
	<title>Property Rate Bill - <?php echo $name; ?></title>
</head>
...
<H2 id="bop">PROPERTY RATE BILL </H2> <strong> <span style="float: right;padding-right: 15px;">DATE: <?php echo date('Y-m-d'); ?></span></strong>
<table border="0" width="620" cellpadding="5">
	<tr>
		<td colspan="3"><strong>BILL TO:</strong> <?php echo $datap['pro_owner_name']; ?></td>
	</tr>
	<tr>
		<td colspan="3"><strong>PROPERTY ACCT. NO:</strong> <?php echo $data['pro_acct_no']; ?></td>
	</tr>
	<tr>
		<td colspan="3"><strong>PROPERTY AREA CATEGORY: </strong><?php echo $datap['pro_area_category']; ?></td>
	</tr>
	<tr>
		<td colspan="2"><strong>ELECTORAL LOCATION / AREA:</strong> <?php echo $datap['electoral_area']; ?></td>
		<td colspan="1"><strong>HOUSE NO.:</strong><?php echo $datap['pro_hs_no']; ?></td>
	</tr>
	<tr>
		<td colspan="2"><strong>LOCATION: </strong><?php echo $datap['pro_address']; ?></td>
		<td colspan="1"><strong>BILL YEAR:</strong> <?php echo date('Y'); //$datap['year']; ?></td>
	</tr>
	<tr>
		<!-- <td colspan="2"><strong>LOCATION</strong> <?php // echo $datap['pro_address']; ?></td> -->
		<td colspan="2"><strong>DISCOUNT of ( <?php echo $data['discount']; ?>% ) : GHC </strong><?php  echo number_format( $data['actual_discount'], 2, '.', ','); ?></td>
	</tr>
</table>
<table border="0" width="620" cellpadding="5">
	<tr>
		...
		<td><strong>RATABLE </strong></td>
		<td><strong>IMPOST</strong></td>
		<td><strong>CURRENT (GHS)</strong></td>
	...
	<tr>
		<td>PR</td>
		<td>Property Rate</td>
		<td><?php echo number_format($data['rateable_value'], 2, '.', ','); ?></td>
		<td><?php echo $data['rate_impose']; ?></td>
		<td><?php echo number_format($data['rate_charge'], 2, '.', ','); ?></td>
		<td><?php echo number_format($data['bal'], 2, '.', ','); ?></td>
		<td><?php
		if($data['p_total'] > 0 && $data['rate_charge'] > $data['p_total']){
			echo number_format( $data['bal'] - $data['p_total'], 2, '.', ',');
		}else{
			echo '0';
		}
		?>
