     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['sd']){
                  ...
                  $sqlnewt = "SELECT sum(paid) as p_total, id, b_permit_acct_no FROM building_payment WHERE date_ >= '$sd' AND date_ <= '$ed' AND activ = 'Y' order by id asc";
                  $resultnewt = $connect->query($sqlnewt);
                  $datat = $resultnewt->fetch_assoc();

                   $sql = "SELECT * FROM building_payment WHERE date_ >= '$sd' AND date_ <= '$ed' AND activ = 'Y' order by id asc";
...
<body>
	...
	<H1 id="bop">BUILDING PERMIT REPORT </H1> <strong> <span style="float: right;padding-right: 15px;">GENERATED ON: <?php echo date('Y-m-d'); ?></span></strong>
	<br><br>
<table id="topic" border="0" width="620" cellpadding="5">
	<tr>
		...
		<td colspan="3"><strong>PAYMENTS COLLECTED (Value):</strong><br> <h1>GHS <?php if($datat['p_total']){echo bcadd($datat['p_total'],0,2);}else{echo "0";}?></h1></td>
