     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $user_id    =  $_SESSION['user_data']['id'];
          ...
          if ($_GET['sd']){
                  ...
                  $sqlnewt = "SELECT sum(paid) as p_total, id, pro_acct_no FROM property_payment WHERE date_ >= '$sd' AND date_ <= '$ed' AND activ = 'Y' order by id asc";
                  $resultnewt = $connect->query($sqlnewt);
                  $datat = $resultnewt->fetch_assoc();

                   $sql = "SELECT * FROM property_payment WHERE date_ >= '$sd' AND date_ <= '$ed' AND activ = 'Y' order by id asc";
...
<body>
	...
	<H1 id="bop">PROPERTY RATE REPORT </H1> <strong> <span style="float: right;padding-right: 15px;">GENERATED ON: <?php echo date('Y-m-d'); ?></span></strong>
	<br><br>
