     ...
     if(isset($_SESSION['user_data']['user_role']) == "agent" || $_SESSION['user_data']['user_role'] == "adminxz"){
        ...
        $last_name  =  $_SESSION['user_data']['last_name'];
                    ...
                    if($_GET['b_no']) {
                                ...
                                $sql = "SELECT * FROM business WHERE b_no = '$id'";
                                $result = $connect->query($sql);

                                $data = $result->fetch_assoc();

                                $sql_ = "SELECT * FROM bop_payment WHERE b_no='$id' AND activ = 'Y' order by id DESC";
...
<head>
    ...
    <title>Search Result | <?php echo $name; ?></title>
    <!-- Favicon-->
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="block-header">
                ...
                <a href="search" class="btn bg-pink waves-effect"> Back </a>
                
                 <a class="btn bg-light-green waves-effect" href='print-bill-bop?id=<?php echo $id_; ?>&b_no=<?php if($data['b_no']){echo $data['b_no'];}else{echo "";} ?>'>Print</a>
                 <a class="btn bg-orange waves-effect" href='update-business.php?id=<?php if($data['id']){echo base64_encode($data['id']);}else{echo "";} ?>'>Edit</a>
                 <?php if($_SESSION['user_data']['user_role'] == "adminxz"){ ?>
                    <a  class="btn bg-primary waves-effect" href='update-business-charge.php?id=<?php if($data['id']){echo base64_encode($data['id']);}else{echo "";}  ?>'>Correct Amt</a>
                 <?php } ?>                    
            </div>
